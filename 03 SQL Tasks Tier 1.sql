...
WITH r AS (
	SELECT f.name, 
		CASE WHEN b.memid = 0 THEN f.guestcost * b.slots
		ELSE f.membercost * b.slots END AS rev
	FROM Bookings AS b
		LEFT JOIN Facilities AS f
		ON b.facid = f.facid
)
SELECT name, SUM(rev) AS revenue
FROM r
GROUP BY name
HAVING revenue < 1000
ORDER BY revenue;
...
SELECT f.name, COUNT(DISTINCT memid) AS usage_by_mem
FROM Bookings AS b
	LEFT JOIN Facilities AS f
	ON b.facid = f.facid
WHERE b.memid != 0
GROUP BY f.name
ORDER BY usage_by_mem DESC;

/* Q13: Find the facilities usage by month, but not guests */

SELECT f.name, strftime('%m', b.starttime) AS month, COUNT(*) as uses
FROM Bookings AS b
	LEFT JOIN Facilities AS f
	ON b.facid = f.facid
WHERE b.memid != 0
